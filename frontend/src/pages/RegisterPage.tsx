...
// ... (declaraciones de 'google' y 'GoogleCredentialResponse' sin cambios)
declare global {
  interface Window {
    google: any;
  }
...
const RegisterPage: React.FC = () => {
  // ... (toda la lógica, hooks y funciones 'handle...' se mantienen exactamente igual)
  ...
  const handleGoogleLoginClick = () => {
    if (window.google?.accounts?.id) {
      ...
      setMessage("El servicio de Google no está disponible. Recarga la página.");
    }
  };
    ...
    <Layout>
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-[#0b0b0b] via-[#151515] to-[#0b0b0b] py-20">
        <div className="bg-gradient-to-r from-[#0b0b0b]/90 via-[#151515]/90 to-[#0b0b0b]/90 p-8 rounded-2xl shadow-lg border border-white/10 w-full max-w-md backdrop-blur-sm">
          {/* ... (Todo el JSX del título, mensajes y formulario se mantiene igual hasta el separador) ... */}
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex justify-center items-center py-3 px-4 border border-white/30 rounded-2xl shadow-lg text-sm font-medium text-gray-100 bg-black/20 hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#151515] focus:ring-white transition-all duration-300 disabled:opacity-50"
          >
